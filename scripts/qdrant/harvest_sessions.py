...
SESSIONS_DIR = Path(os.environ.get("OPENCLAW_AGENTS_DIR", os.path.expanduser("~/.openclaw/agents")))
...
def main():
    ...
    # Find all session files (active + reset) across all agents
    session_files = sorted(
        [f for f in SESSIONS_DIR.rglob("*.jsonl*") 
    ...
    for i, session_file in enumerate(session_files, 1):
        # Extract agent name from path (e.g. /agents/commander-rin/sessions/...)
        ...
        print(f"\n[{i}/{len(session_files)}] Processing: {agent_name}/{session_file.name}")
