...
User-centric memory - all conversations link to persistent agent_id.
NOT session/chat-centric like old version.

Features:
- Persistent agent_id (e.g., "rob") across all conversations
...
Mem0 Architecture:
    - agent_id: "rob" (persistent across all your chats)
    - conversation_id: Groups turns within one conversation
    - session_id: Optional - tracks specific chat instance
    - Retrieved by: agent_id + semantic similarity (NOT session_id)
"""
...
def is_duplicate(agent_id: str, user_msg: str, ai_response: str) -> bool:
    """
    Check if this conversation turn already exists for this user.
    Uses: agent_id + content_hash
    """
    ...
    # Check Qdrant for existing entry with this agent_id + content_hash
    try:
        search_body = {
            "filter": {
                "must": [
...
def store_memory_point(
    ...
    """Store a single memory point to Qdrant with agent_id"""
    ...
    payload = {
        # MEM0-STYLE: agent_id is PRIMARY key
        "agent_id": agent_id,
        "text": text,
        "date": date_str,
        "tags": tags,
...
def store_conversation_turn(
    ...
    Args:
        agent_id: PERSISTENT user identifier (e.g., "rob") - REQUIRED
    ...
    """
    if not agent_id:
        raise ValueError("agent_id is required for Mem0-style storage")
    
    if date_str is None:
        date_str = datetime.now().strftime("%Y-%m-%d")
    
    # Check for duplicates (per user)
    if skip_if_duplicate and is_duplicate(agent_id, user_message, ai_response):
        return {
            "user_point_id": None,
            "ai_point_id": None,
            "agent_id": agent_id,
    ...
    # Tags include agent_id for easy filtering
    tags = [
        "conversation",
        f"user:{agent_id}",
        date_str
    ...
    # Store user message
    user_text = f"[{agent_id}]: {user_message}"
    agent_id_point = store_memory_point(
        agent_id=agent_id,
        text=user_text,
        speaker="user",
        date_str=date_str,
    ...
    # Mark as stored
    if agent_id_point and ai_id_point:
        mark_stored(user_message, ai_response)
    
    return {
        "user_point_id": agent_id_point,
        "ai_point_id": ai_id_point,
        "agent_id": agent_id,
        "conversation_id": conversation_id,
        "turn_number": turn_number,
        "success": bool(agent_id_point and ai_id_point),
        "skipped": False
    }

def main():
    parser = argparse.ArgumentParser(
        description="Auto-store conversation turns to Qdrant (TRUE Mem0-style with agent_id)"
    )
    ...
    if result.get("skipped"):
        print(f"⚡ Skipped duplicate (already stored for user {result['agent_id']})")
    elif result["success"]:
        print(f"✅ Stored for user '{result['agent_id']}' turn {result['turn_number']}")
